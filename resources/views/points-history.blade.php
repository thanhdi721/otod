                ...
                <div class="list-group points-list">
                    ...
                    @foreach ($items as $item)
                        <div class="list-group-item p-3 d-flex align-items-center justify-content-between">
                            <div>
                                <div class="fw-semibold">{{ $item['id'] }} {{ $item['text'] }}</div>
                                <div class="text-muted small">{{ $item['time'] }}</div>
                            </div>
                            <div class="d-flex align-items-center gap-2">
                                <div class="fw-bold {{ str_starts_with($item['delta'], '+') ? 'text-success' : 'text-danger' }}">
                                    {{ $item['delta'] }}
                                </div>
                                <img src="{{ asset('/assets/images/ic-poin.png') }}" alt="coin" width="16" height="16">
                            </div>
                        </div>
                    @endforeach
                </div>
