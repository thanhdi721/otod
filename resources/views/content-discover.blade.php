...
<div class="container py-5">
  ...
  <ul class="nav nav-pills mb-4 gap-2" id="contentTabs">
    ...
    @foreach($tabs as $tab)
      <li class="nav-item">
        <button class="nav-link tab-btn {{ $tab['active'] ? 'active' : '' }}" 
                data-tab="{{ $tab['id'] }}"
                style="color: {{ $tab['active'] ? '#fff' : '#000' }}; 
                       background-color: {{ $tab['active'] ? '#2B4896' : '#F4F4F4' }}; 
                       border-radius: 24px;">
          {{ $tab['name'] }}
        </button>
      </li>
...
?>
          @foreach($storyUsers as $user)
            <div class="story-card" style="min-width: 140px;">
              <img src="{{ asset('/assets/images/img-story.png') }}" class="bg" alt="">
              <div class="avatar"><img src="{{ asset('/assets/images/avatar.png') }}" alt=""></div>
...
?>
  ...
  <div class="all-sections active">
    @foreach($sections as $section)
      <div class="mt-5">
        <h3 class="fw-bold mb-4">{{ $section['title'] }}</h3>
        
        @if($section['id'] === 'noi-bat')
          ...
          <div class="highlight-wrapper row g-4 flex-md-row flex-nowrap overflow-auto" style="scrollbar-width: none; -ms-overflow-style: none;">
            ...
            @foreach($section['cards'] as $card)
              <div class="col-md-3 col-8">
                <a href="{{ route('content.detail', ['id' => $loop->index, 'section' => $section['id'], 'title' => urlencode($card['title']), 'date' => $card['date']]) }}" class="text-decoration-none">
                  <div class="highlight-card">
                    <img src="{{ asset('/assets/images/img-prd.png') }}" alt="{{ $card['title'] }}">
                    <div class="card-body">
                      <h5>{{ $card['title'] }}</h5>
                      <div class="date-info">
  ...
  @foreach($sections as $section)
    @if($section['id'] !== 'noi-bat')
      <div class="single-section" id="single-section-{{ $section['id'] }}">
        <div class="mt-5">
          <h3 class="fw-bold mb-4">{{ $section['title'] }}</h3>
          <div class="row g-4">
            @foreach($section['cards'] as $card)
