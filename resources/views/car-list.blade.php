    ...
    <div class="container w-100" style="margin-left: auto; max-width: 800px">
        ...
        <div class="tag-status-type mb-3">
            <div class="d-flex gap-3">
                <a href="{{ route('car-list', ['status' => 'renting']) }}"
                    class="rounded-4 py-1 px-3 text-decoration-none {{ $currentStatus === 'renting' ? 'active-status' : 'inactive-status' }}">
                    Đang cho thuê
                </a>
                <a href="{{ route('car-list', ['status' => 'pending']) }}"
                    class="rounded-4 py-1 px-3 text-decoration-none {{ $currentStatus === 'pending' ? 'active-status' : 'inactive-status' }}">
                    Chờ duyệt xe
                </a>
            </div>
        </div>
        ...
        @if (empty($cars))
            ...
            <div class="d-flex flex-column align-items-center justify-content-center p-5 rounded-4"
                ...
                <div>
                    <img src="{{ asset('/assets/images/car-list.png') }}" alt="car">
                </div>
                ...
                <button class="btn-add-new-car rounded-3 px-4 py-2 fw-medium">
                    <img src="{{ asset('/assets/images/plus-ic.png') }}" alt=""> Thêm xe mới
                </button>
            </div>
        @else
            <!-- List Cars -->
                    ...
                    <div class="car-item mt-4 d-flex flex-column gap-3 border p-3 rounded-4">
                        <div class="car-content d-flex justify-content-between mb-2 gap-2">
                            <div class="car-info d-flex gap-3">
                                <img src="{{ asset($car['image']) }}" alt="car" class="object-fit-cover"
                                    style=" height:76px; border-radius:8px">
                                <div class="d-flex flex-column justify-content-center">
                                    <div class="fw-bold mb-1 fs-5 fs-md-4 fs-lg-3">{{ $car['name'] }}</div>
                                    <div style="color:#737272">{{ $car['type'] }}</div>
                                    <div class="fw-medium">{{ $car['price'] }}</div>
                                </div>
                            </div>
                            <div>
                                <span class="text-muted fs-6 fs-md-5 fs-lg-4 p-2"
                                    style="background: #F4F4F4">{{ $car['license_plate'] }}</span>
                            </div>
                        </div>
                        <hr style="margin: 0.5rem 0 !important">
                        <div class="d-flex flex-column gap-2">
                            <div class="d-flex gap-2 align-items-center">
                                <img src="{{ asset('/assets/images/location-ic-list-car.png') }}" alt="">
                                <span>{{ $car['location'] }}</span>
                            </div>
                            <div class="d-flex gap-3">
                                <div class="d-flex gap-1 align-items-center">
                                    <img src="{{ asset('/assets/images/number-auto-ic.png') }}" alt="">
                                    <span>{{ $car['transmission'] }}</span>
                                </div>
                                <div class="d-flex gap-1 align-items-center">
                                    <img src="{{ asset('/assets/images/seat-ic.png') }}" alt="">
                                    <span>{{ $car['seats'] }}</span>
                                </div>
                                <div class="d-flex gap-1 align-items-center">
                                    <img src="{{ asset('/assets/images/gasoline-ic.png') }}" alt="">
